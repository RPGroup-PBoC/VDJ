"""
Cutting Probability Distributions for Endogenous RSSs
--------------------------------------------------------------------------------
...
Description
--------------------------------------------------------------------------------
This figure generates a ridgeplot of the endogenous RSS posterior
distributions for the cleavage probability.
...
# Keep only the endogenous mutants.
df = []
for g, d in cut_posts.groupby(['mutant']):
    if ('Hept' not in g) & ('Spac' not in g) & ('Non' not in g) & ('Cod' not in g):
        df.append(d) 
endo_cuts = pd.concat(df)


# Properly rename columns for display
...
ax.text(0.74, offset_dict["DFL16.1-3'"] + 0.012, '$\propto$probability', 
         fontsize=12)
ax.set_xlim([0.0, 1.0])
ax.set_yticklabels([])
ax.set_facecolor('white')
ax.set_xlabel('$p_{cut}$', fontsize=14)
plt.savefig('../../figures/SiFig_endogenous_posterior_ridgeplot.pdf',
