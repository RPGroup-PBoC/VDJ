...
Description
--------------------------------------------------------------------------------
This script generates a stick plot comparing various properties (looping
frequency, median dwell time, and cutting probability) of the selected
endogenous RSSs.

Notes
--------------------------------------------------------------------------------
This script is designed to be executed from the `code/figures` directory and
uses a relative path to load the csv files.
"""
import numpy as np
import pandas as pd
...
cuts = pd.read_csv('../../data/pooled_cutting_probability.csv', comment='#')
cuts = cuts[(cuts['salt']=='Mg') & (cuts['hmgb1']==80) &
                (cuts['mutant']!='12CodC6A')]
dwell = pd.read_csv('../../data/compiled_dwell_times.csv', comment='#')
dwell = dwell[(dwell['salt']=='Mg') & (dwell['hmgb1']==80) &
                 (dwell['mutant']!='12CodC6A')]
bs_loops = pd.read_csv('../../data/compiled_looping_frequency_bootstrap.csv', 
                        comment='#')
counts = bs_loops[(bs_loops['salt']=='Mg') & (bs_loops['hmgb1']==80) &
                 (bs_loops['mutant']!='12CodC6A') & 
                 (bs_loops['percentile']==95.0)]
...
cuts.rename(columns={'mean':'mean_p_cut'}, inplace=True)
...
plt.savefig('../../figures/Fig_endogenous_properties.pdf', facecolor='white', 
                bbox_inches='tight')
