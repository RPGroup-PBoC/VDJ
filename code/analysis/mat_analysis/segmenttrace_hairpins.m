function [statetrace,allstates]=segmenttrace_hairpins(trace,threshold,trackthresh,stickthresh,deadfilter)
offstates_label=[];
onstates_label=[];
trackerror_label=[];
stuck_label=[];
...
if spurfirst==1
    ...
    allstates_prop=regionprops(allstates);
    sss=0;
    while (sum([allstates_prop.Area]<2*deadfilter))>0 && sss<10
        sss=sss+1
    [allstates,statetrace]=removeshort(allstates,statetrace,deadfilter);   
    allstates_prop=regionprops(allstates);
    end
    ...
    %let's make a new state for loss

    %check to make sure it isn't 100% unhinged
    if sum(statetrace<4)>0
    [allstates,statetrace]=removespur(allstates,statetrace);
    end
end
...
% while statetrace(round(allstates_prop(laststateindex).Centroid(1)))==4
%         allstates_prop(laststateindex).Centroid(1)
...
for i=1:size(allstates_prop,1)
    if allstates_prop(i).Area<2*deadfilter && allstates_prop(i).Centroid(1)<laststate && allstates_prop(i).Centroid(1)>firststate
        ...
        allstates(ceil(cent-halfwidth):ceil(cent-1))=allstates(ceil(cent-halfwidth)-1);
        allstates(ceil(cent):floor(cent+halfwidth))=allstates(floor(cent+halfwidth)+1);
    elseif allstates_prop(i).Area<2*deadfilter && allstates_prop(i).Centroid(1)==laststate && statetrace(round(allstates_prop(i).Centroid(1)))>1
