%function converttoHMMformat_metrop()
%
...
for i=1:totsets
    ...
    %Update 11/2011: I'm pretty sure the only zero entries are the ones where 
    %part of a bead was discarded.  Using find() to find cutpoints
    ...
    if ~isfield(lac,'cutpts')
        ...
        thislacX = templacX;
        thislacY = templacY;
        clear tempnolacX tempnolacY templacX templacY r
        
        %if ~isempty(thislacX{x}) %second sets are handled below
        cutpts{i}=zeros(1,size(thislacX,1));
        laccurrbd = 1; %LacX and lacANALRMS have different lengths
            for x = 1:size(thislacX,1) 
                %Need to use record to figure out if any beads were
                %discarded; but record wasn't always saved properly ...
                %Think this mainly matters for second sets though
                if lac.record{i}(x)==1 %Some part of the bead was kept
                    cutpts{i}(x) = length(find(lac.lacANAL_RMS{i}(laccurrbd,:)));
                    laccurrbd = laccurrbd+1;
                else
                    cutpts{i}(x) = 0;
                end
            end
    ...
    if length(lac.lacX) > totsets %Means there was at least one second set
        ...
        if ~isfield(lac,'cutpts')
            if ~isempty(thislacX2)
                ...
                cutpts{i+totsets}=zeros(1,size(thislacX2,1));
                laccurrbd = 1; %LacX and lacANALRMS have different lengths
                for x = 1:size(thislacX2,1) 
                    %I think what happened with record is that it didn't
                    %create empty matrices for any empty second sets beyond
                    %the last real second set.  So this should still work.
                    if lac.record{i+totsets}(x)==1 %Some part of the bead was kept
                        cutpts{i+totsets}(x) = length(find(lac.lacANAL_RMS{i+totsets}(laccurrbd,:)));
                        laccurrbd = laccurrbd+1;
                    else
                        cutpts{i+totsets}(x) = 0;
                    end
                end
